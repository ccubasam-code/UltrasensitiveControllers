...
bc = 3*10^4*uMh;
ac = 3*10^4*uMh;
% phc = log(2)*60/30; % protein degradation due to dilution
phc = log(2)*60/30/1; % RNA degradation due to dilution
gc = 3*10^4*uMh;
ut = 0.5;
...
p(7) = p(7)*2;
[t3,s3] = ode23s(@(t,x) ODE_Model_PIControler_ModuleI(t,x,p),tspan,x01,options);
t0 = [t0 ;t3+ t0(end)];
s0 = [s0;s3];

% figure(1)
% subplot(2,1,1),plot(t0,s0(:,4),'r',t1,s1(:,5),'b',t1,s1(:,1),'k','LineWidth',2)
% legend('g_1','A_1','u')
% 
% subplot(2,1,2),plot(t0,s0(:,2),'r',t1,s1(:,3),'b',t1,s1(:,6),'k','LineWidth',2)
% legend('y','r','r_u')
% 
% figure(2)
% plot(t1,s1(:,4),'k',t1([1,end]),[r1 r1],'r--',t1([1,end]),[r2 r2],'r--','LineWidth',2)
% xlabel('time (h)')
% ylabel('g_y (\mu M)')
